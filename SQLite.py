...
class DB(object):
    ...
    def connect(self):
        ...
        if self.connection is not None:
            return
        try:
            self.connection = mdb.connect("library.db")
        except mdb.enable_callback_tracebacks as e:
            self.connection = None
    ...
    def get_authors(self):
        ...
        # cur = self.connection.cursor(mdb.Cursor.)
        con = mdb.connect("library.db")
        cur = con.cursor()
        #cur = con.cursor()
...
#
    def add_author(self, fname, lname):
        ...
        cur = con.cursor()
        #cur = con.cursor()
        cur.execute('''INSERT INTO author VALUES (?, ?, ?, ?)''', (19, fname, lname, 215))
        con.commit()
        self.close()
        #return cur.fetchall()
